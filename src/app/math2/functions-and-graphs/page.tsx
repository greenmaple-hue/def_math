...
export default function FunctionsAndGraphsPage() {
  ...
  const getChapterName = (mainTitle: string, subTitle: string) => {
    return `공통수학 2 - 함수와 그래프 - ${mainTitle} - ${subTitle}`;
  };
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      <header className="bg-white border-b border-gray-200 px-8 py-4 flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-bold text-gray-900">III. 함수와 그래프</h1>
      ...
      <main className="flex-1 max-w-5xl w-full mx-auto p-8">
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-sky-200 overflow-hidden ring-1 ring-sky-50">
            ...
            >
              <span className={`text-lg font-bold ${expandedChapter === 1 ? "text-sky-900" : "text-gray-900"}`}>1. 함수</span>
                  ...
                  <div key={sub.id} className={`rounded-xl overflow-hidden border ${expandedSub === sub.id ? "bg-sky-50/50 border-sky-100" : "bg-gray-50 border-gray-100"}`}>
                    ...
                    {expandedSub === sub.id && (
                      <div className="p-6 pt-2">
                        <AiQuizForm chapter={getChapterName("1. 함수", sub.title)} />
                      </div>
                    )}
                  </div>
                ))}
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-sky-200 overflow-hidden ring-1 ring-sky-50">
            <button 
              ...
              className="w-full flex items-center justify-between p-6 hover:bg-gray-50 transition-colors text-left"
            >
              <span className={`text-lg font-bold ${expandedChapter === 2 ? "text-sky-900" : "text-gray-900"}`}>2. 유리함수와 무리함수</span>
              ...
              <div className="px-6 pb-6 space-y-3">
                {[
                  ...
                  <div key={sub.id} className={`rounded-xl overflow-hidden border ${expandedSub === sub.id ? "bg-sky-50/50 border-sky-100" : "bg-gray-50 border-gray-100"}`}>
                    ...
                    {expandedSub === sub.id && (
                      <div className="p-6 pt-2">
                        <AiQuizForm chapter={getChapterName("2. 유리함수와 무리함수", sub.title)} />
                      </div>
                    )}
                  </div>
                ))}
